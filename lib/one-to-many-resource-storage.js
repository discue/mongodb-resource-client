...
module.exports = class extends Base {
    ...
    constructor(options) {
        ...
        this._resourcePath = (options.resourcePath || options.resourceName).split('/').filter(s => s)
     ...
     * @param {boolean} [withMetadata=false] true if also _meta_data object should be returned
     * @returns {Object}
     */
    async get(resourceIds, withMetadata = false) {
        const resource = await super.get(resourceIds.slice(-1), withMetadata)
        if (resource != null) {
            const hosts = await this._hostStorage.findReferences(this._getResourceIdsForHostStorage(resourceIds), [resource.id])
            const isReferenced = hosts.length > 0
            if (isReferenced) {
                return resource
            }
        }
     ...
     */
    async getAll(resourceIds, withMetadata = false) {
