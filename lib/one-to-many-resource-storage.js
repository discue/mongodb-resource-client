...
module.exports = class extends Base {
    ...
    async _getResourceWithLookup(resourceIds, options = {}) {
        if (!Array.isArray(resourceIds) || resourceIds.length - 1 !== this._resourcePath.length) {
            console.error('Given IDS and resource path dont match lengths', { resourceIds, resourcePath: this._resourcePath })
            throw new Error('Ids and patch dont match')
        }
        ...
        let object = result.at(0)
        for (let i = 1, n = this._resourcePath.length; object && i < n; i++) {
            object = object[this._resourcePath.at(i)]
        ...
        // if
        // - object is not null 
        // - and has a key name $resourceName
        // - and value of $resourceName is an array and has at least one element
        // - then return array element at index 0
        // - otherwise return null.
        object = object?.[this._resourceName]
        return object?.at(0) ?? null
    }
    ...
    async getAll(resourceIds, { withMetadata = false, addDocumentPath, projection } = {}) {
        return withActiveSpan(`${name}#get-all-one-to-many-resources-by-id`, { resourceIds, resourceName: this._collectionName, databaseName: this._databaseName }, async () => {
            ...
            const resources = object[this._resourceName]
            if (!resources?.length) {
                return []
            }
