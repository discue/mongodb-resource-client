...
module.exports = class extends Base {
    ...
    async getAll(resourceIds, { withMetadata = false, projection } = {}) {
        ...
        const localCollectionIds = references.map((refs) => refs.at(-1))
        const aggregationStages = [
            EQUALS_ANY_OF('id', localCollectionIds)
        ]
    ...
    async findReferences(resourceIds, verifyParentReference = true) {
        ...
        if (resourceIds.length === this._resourcePath.length) {
            return refs.map(ref => [...resourceIds, ref])
        }

        const resolvedRefs = []
        for (let i = 0, n = refs.length; i < n; i++) {
            const r = await this.findReferences([...resourceIds, refs.at(i)], false)
            resolvedRefs.push(...r)
        }
        return resolvedRefs
    }
