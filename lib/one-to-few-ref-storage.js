...
module.exports = class extends Base {
    ...
    async exists(resourceIds) {
        return withActiveSpan('exists-one-to-few-ref-by-id', { resourceIds, resourceName: this._resourceName, databaseName: this._databaseName }, async () => {
    ...
    async getAll(resourceIds) {
        return withActiveSpan('get-all-one-to-few-refs-by-id', { resourceIds, resourceName: this._resourceName, databaseName: this._databaseName }, async () => {
            const collection = await this._getCollection()
            const parent = await collection.findOne({
                id: resourceIds.at(0)
            })
    ...
    async create(resourceIds, ref, options) {
        return withActiveSpan('create-one-to-few-ref-by-id', { resourceIds, resourceName: this._resourceName, databaseName: this._databaseName }, async () => {
    ...
    async findReferences(resourceIds, references) {
        return withActiveSpan('find-one-to-few-refs-by-id', { resourceIds, resourceName: this._resourceName, databaseName: this._databaseName }, async () => {
            const collection = await this._getCollection()
    ...
    async delete(resourceIds, options) {
        return withActiveSpan('delete-one-to-few-ref-by-id', { resourceIds, resourceName: this._resourceName, databaseName: this._databaseName }, async () => {
            const exists = await this.exists(resourceIds)
            if (!exists) {
